...
- `Survey_Matrices_Car_Bus_Rail_Report.docx` — plain-language report on the survey-only car / bus / rail matrices: methodology, the similarity tests (cosine / GEH, KS, MSSIM), the RavKav × OnBoard calibration and its coverage finding, the move to 2022, the corridor demand profiles with their comparison against the ticketing-based profile and the tests against the cellular hybrid, and the one-hour reporting recommendation
