...
(`Output/gc/gc_data_inventory.csv`, task list section E). Step 29 (§6aa) adds the bus and BRT level of service per TAZ from the national GTFS
(dry run until the LFS archive is pulled). The plain-language account of steps
...
| `../GTFS/israel-public-transportation.zip` (in `Input/`, LFS, expected 22 Sep 2026) | The Ministry of Transport's national GTFS feed (https://gtfs.mot.gov.il/gtfsfiles/) | Standard GTFS (`stops`, `routes`, `trips`, `stop_times`, `calendar`, `agency` …); `route_desc` carries the route code (מק"ט), direction and alternative as `code-dir-alt`; the Metronit BRT lines are codes 83001–83005. Used by step 29 for the bus level of service per TAZ and the direct-service skim between the V2 areas; the notebook dry-runs on a synthetic feed while only the LFS pointer is present |
...
**Status.** The GTFS archive is Git-LFS-only and the MOT server is not reachable from the
remote environment, so the notebook was executed as a **dry run** on a synthetic feed in
the national format (16 stops, two bus lines and one line under code 83001, 120 trips);
every step ran and the outputs under `Output/gtfs/dry_run/` are the mechanics test. The
real run — `git lfs pull --include="Input/GTFS/israel-public-transportation.zip"`, then
execute — writes `Output/gtfs/bus_los_taz.csv`, `Output/gtfs/bus_direct_skim_area_v2.csv`
and `Output/figures/gtfs_bus_los_taz.png`. Its first reading should be the study-area trip
count on the chosen Tuesday, the BRT-access table by V2 area, and the ratio of scheduled
direct in-vehicle time to the §6x floor (the survey's door-to-door time was 2.1–2.3 × that
floor).

**Outputs.** `Output/gtfs/bus_los_taz.csv` (781 rows), `Output/gtfs/bus_direct_skim_area_v2.csv`,
figure `gtfs_bus_los_taz.png` (under `dry_run/` and `_dry_run` until the real run).
...
# bus level of service from the national GTFS (step 29; dry run on a synthetic feed unless git lfs pull --include="Input/GTFS/israel-public-transportation.zip")
